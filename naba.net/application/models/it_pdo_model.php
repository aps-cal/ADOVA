<?php
class IT_pdo_model extends CI_Model {
   //$data = array();

    public function __construct()	{
        $this->load->database('pdo', true);
   ...
   /*       
            ...
            if(isset($data['Serial_No']) AND $data['Serial_No']!==''){
                ...
                $filter.="AND Serial_No = :Serial_No ";
            ...
            if(isset($data['Computer_Name']) AND $data['Computer_Name']!==''){
                ...
                $filter.="AND Computer_Name = :Computer_Name ";
                $filterItems['Computer_Name'] = $data['Computer_Name'];
            }
            if(isset($data['Tag_No']) AND $data['Tag_No']!==''){
                //$filter.="AND Tag_No = '".$data['Tag_No']."' ";
                $filter.="AND Tag_No = :Tag_No ";
                $filterItems['Tag_No'] = $data['Tag_No'];
            }
     */
            // If a filter has been set preserve the values 
            $this->session->unset_userdata('filterStr');
            
            
            // *****************
            // This following line needs re-thinking as it no longer works with the new coding
            // *******************
          
    /*        
            if($filterStr!==$defaultStatus){   
                //$this->session->set_userdata('filter', $filter);
                $this->session->set_userdata('filterStr', $filterStr);
                $this->session->set_userdata('filterItems', $filterItems);
            } elseif($this->session->userdata('filterStr')) {
                
                //$filter = $this->session->userdata('filter');
                $filterStr = $this->session->userdata('filterStr');
    ...
    */        echo "Original Completed<br/>";
            ...
            echo "Got Here!<br/>";
            foreach($filterItems as $key => $value ) {
             //   $statement->bindParam(concat(':',$key), $value, PDO::PARAM_STR);
            }
            echo "Got Here!<br/>";
           //$statement->bindParam(':ListOrder', $ListOrder, PDO::PARAM_STR);          
            $statement->execute();
            $computers = $statement->fetchAll();
            $data['computers'] = $computers;

        }catch (Exception $e) {
            echo "Error in GetComputers() ...";
            echo $e->message();
            echo var_dump($sql);
        }
        return($data);
