...
class OAuth_model extends CI_Model {
   ...
   private $request_token;
	...
	public function __construct()	{
      ...
      $this->signatures = array('consumer_key' => 'troja.csv.warwick.ac.uk',
...
//              .'target=http://www.mycalonline.org.uk';
      $url = 'https://websignon.warwick.ac.uk/origin/logout?'
...
//               'oauth_callback'=> 'http://www.mycalonline.org.uk/login/oauthlogin'),
//            'signatures'=> $signatures));
         $result = $oauthObject->sign(array(
            'path'      =>'https://websignon.warwick.ac.uk/oauth/requestToken',
            'parameters'=> array(
               'scope'         => 'urn:websignon.warwick.ac.uk:sso:service',
               'oauth_callback'=> 'http://aldb.warwick.ac.uk/login/oauthlogin'),
            'signatures'=> $signatures));
         ...
         curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); // Should prevent output to browser
         $r = curl_exec($ch);
         curl_close($ch);
