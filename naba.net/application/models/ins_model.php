...
//      $this->email->initialize($config);  
   ...
   public function GetSubmissions($data){
       ...
       * report received from SiteBuilder onto the correct field in the "xxx_Submissions" table.
       */
...
$pageURL = "https://sitebuilder.warwick.ac.uk/sitebuilder2/forms/submissions/download.xml?page=/fac/soc/al/intranet/learn_english/in-sessional/late_registration&startDate=05/01/2015&endDate=05/10/2015&filter=warwick"; 
// SiteBuilder paths were changed with new web-site Dec 2015
$pageURL = "https://sitebuilder.warwick.ac.uk/sitebuilder2/forms/submissions/download.xml?page=/fac/soc/al/study/learn-english/in-sessional/late_registration/&startDate=05/01/2015&endDate=05/10/2015&filter=warwick"; 
$ch = curl_init($pageURL); 
// Print any response headers 
...
if (TRUE){
      try {
         $sql = "SELECT Form_ID, Form_URL FROM online_forms "
           ."WHERE Programme = ? AND Form_Enabled = 1 "
           ."ORDER BY Form_Name ";
           $query = $this->db->query($sql, array($programme));
           $FormCount = 0;
    //  echo var_dump($sql);
         foreach($query->result_array() as $row){
            $pageURL = "https://sitebuilder.warwick.ac.uk/sitebuilder2/forms/submissions/download.xml?"
               ."page=".$row['Form_URL']
               ."&startDate=".date('d/m/Y',time()-(60*60*24*300))
               ."&endDate=".date('d/m/Y', time()+(60*60*24))."&filter=warwick&forceBasic=true";
            
    //      echo $pageURL; 
          echo var_dump($pageURL);
            $xmlDoc = $this->ins_model->GetXML($pageURL);
    //      echo "XMLDOC = {".var_dump($xmlDoc)."}";
  
            if(is_string($xmlDoc)){
               $xmlDoc = str_replace("<filter/>", "<filter></filter>", $xmlDoc);
               $this->XML2Record($xmlDoc,$row['Form_ID']);
            }
...
//            $row = $query->row_array();
         ...
         // Update the new records added to Submissions with the current Academic Year/Term
         $sql = "UPDATE ins_submissions S, current_values CV "
         ."SET S.Academic_Year = CV.Academic_Year, S.Academic_Term = CV.Academic_Term "
          ...
          * If Err = -2147352571 Then ' Type Mismatch (probably a date format)
          */
      }
}      
      $data['List_Order'] = (isset($_POST['List_Order'])?$_POST['List_Order']:'Submission_ID DESC');
      $sql = "SELECT S.* FROM ins_submissions S "
    ...
    public function GetXML ($pageURL){
        error_reporting(E_STRICT); 
        try{
        ...
        //echo "GOT HERE";
         ...
         $xmlHttpRequest = new HttpRequest($pageURL);
         $username = 'el-apiuser';
         $password = 'Roberts1951';
         $xmlHttpRequest->setUrl($pageURL);
         $Options = array("httpauth" => $username.':'.$password, "httpauthtype"=>HTTP_AUTH_BASIC, "timeout" => 30);
         $xmlHttpRequest->setOptions($Options);
        
        $Headers = array();
         $Headers["User-Agent"] = "Andrew P Smith, Centre for Applied Linguistics, 07746412190, andrew.p.smith@warwick.ac.uk";
         $Headers["Content-Type"] = "application/x-www-form-urlencoded";
         $Headers["If-Modified-Since:"] = "Tue, 11 Jul 2000 18:23:51 GMT";
         //echo "GOT HERE";
         $xmlHttpRequest->setHeaders($Headers);
         $xmlHttpRequest->setMethod(HTTP_METH_POST);
    //     echo "<p>";
    //     echo var_dump($xmlHttpRequest);
    //     echo "</p><br><br>";
         $xmlHttpRequest->send();
//         echo var_dump($pageURL);
//         echo var_dump($xmlHttpRequest->getResponseBody());
//         echo var_dump("RAW[".$xmlHttpRequest->getRawRequestMessage()."]");
         while ($xmlHttpRequest->getResponseCode() != 200) {
            if($xmlHttpRequest->getResponseCode()==320) {
               echo "#".$xmlHttpRequest->getResponseCode();
            }
         }
         $xmlDoc = $xmlHttpRequest->getResponseBody();
      }catch (Exception $e){
         echo var_dump($xmlHttpRequest);
         echo '<br>';
         echo 'Error Message: '.$e->getMessage();
         echo '<br>';
         echo 'Error Code: ['.$xmlHttpRequest->getResponseCode().'] ';
         echo '<br>';
         echo "RAW[".(string)($xmlHttpRequest->getRawResponseMessage())."]";
         echo '<br>';
   ...
   public function XML2Record($xmlStr, $FormID){
      
      $sql = "SELECT * FROM ins_submissions";
      $doc = new DOMDocument();
      //doc.SetProperty "ProhibitDTD", False
      //doc.SetProperty "ResolveExternals", False
      //doc.SetProperty "ValidateOnParse", False
      //doc.async = False
      //echo "<code>$xmlStr</code>";
      $doc->loadXML($xmlStr);
      if(!$doc->validate()){
         echo "XML Document is INVALID";
      }
      $processed = FALSE;
      foreach($doc->getElementsByTagName('submission') as $submission ){
         $values = array();
         foreach($submission->getElementsByTagName('property') as $property ){
            $values[$property->getAttribute('name')] = $property->nodeValue;
         }
         $sql = "SELECT 1 FROM ins_submissions WHERE Submission_ID = ? ";
         $query = $this->db->query($sql,(isset($values['Submission ID'])?$values['Submission ID']:0));
         $row = $query->row_array();
         if(!$row){
            //echo var_dump($values);
            $params = array('','',
                (isset($values['Submission ID'])?$values['Submission ID']:''),
                (isset($values['Submission time'])?date('Y-m-d h:n', strtotime($values['Submission time'])):''),
                (isset($values['University_ID'])?$values['University_ID']:''),
                (isset($values['First_Name'])?$values['First_Name']:''),
                (isset($values['Last_Name'])?$values['Last_Name']:''),
                (isset($values['Department'])?$values['Department']:''),
                (isset($values['Status'])?$values['Status']:''),
                (isset($values['Email'])?$values['Email']:''),
                (isset($values['Skills_Reqd'])?$values['Skills_Reqd']:''),
                (isset($values['Skills'])?$values['Skills']:''),
                (isset($values['IELTS_Listening'])?$values['IELTS_Listening']:''),
                (isset($values['IELTS_Speaking'])?$values['IELTS_Speaking']:''),
                (isset($values['IELTS_Reading'])?$values['IELTS_Reading']:''),
                (isset($values['IELTS_Writing'])?$values['IELTS_Writing']:''),
                (isset($values['IELTS_Score'])?$values['IELTS_Score']:''),
                (isset($values['English_Level'])?$values['English_Level']:''),
                (isset($values['Attendance'])?$values['Attendance']:''),
                (isset($values['Referrer'])?$values['Referrer']:''),
                (isset($values['Staff_Name'])?$values['Staff_Name']:''),
                (isset($values['Staff_Dept'])?$values['Staff_Dept']:''),
                (isset($values['Staff_Email'])?$values['Staff_Email']:''),
                (isset($values['Student_Name'])?$values['Student_Name']:''),
                (isset($values['Student_Dept'])?$values['Student_Dept']:''),
                (isset($values['Student_Email'])?$values['Student_Email']:''));
            $sql = "INSERT INTO ins_submissions ("
               ."Academic_Year, Academic_Term, Submission_ID, Submission_Time, "
               ."University_ID, First_Name, Last_Name, Department, "
               ."Status, Email, Skills_Reqd, Skills, IELTS_Listening, IELTS_Speaking, "
               ."IELTS_Reading, IELTS_Writing, IELTS_Score, English_Level, Attendance, "
               ."Referrer, Staff_Name, Staff_Dept, Staff_Email, "
               ."Student_Name, Student_Dept, Student_Email) "
               ." VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
            try{
